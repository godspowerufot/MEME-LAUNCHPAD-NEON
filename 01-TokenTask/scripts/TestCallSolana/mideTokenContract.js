...
async function main() {
    ...
    tx = await ERC20ForSplFactory.createErc20ForSplMintable(
        "codewithmide token " + Date.now().toString(),
        "CWM",
        9,
        user1.address
    );
    ...
    // deploy mideTokenContract contract
    const mideTokenContract = await ethers.deployContract("mideTokenContract", [ERC20ForSplMintable.target]);
    await mideTokenContract.waitForDeployment();
    console.log(`mideTokenContract deployed to ${mideTokenContract.target}`);

    const contractPublicKey = ethers.encodeBase58(await mideTokenContract.getNeonAddress(mideTokenContract.target));
